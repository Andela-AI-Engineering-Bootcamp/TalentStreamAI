...
- **Traces and alerts**: The codebase is **ready** for you to plug OpenTelemetry (e.g. export to your collector when `OTEL_EXPORTER_OTLP_ENDPOINT` is set) and to wire **alerting** on `5xx` rate, p95 latency, LLM error rate, and `tailor_runs_total{outcome="error"}`. The exact exporter packages are left to your platform to avoid unused heavy dependencies in minimal installs.
...
See the repository root `.env.example` for `LOG_LEVEL`, `LOG_JSON`, `ENABLE_PROMETHEUS`, `SERVICE_NAME`, and auth/LLM keys. In **deployed** environments, startup checks still enforce `AUTH_MODE`, `AGENT_MODE`, and `UPLOAD_STORAGE` as in `app/main.py`.
